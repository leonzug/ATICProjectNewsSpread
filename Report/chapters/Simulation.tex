...
\begin{figure}[!t]
	\centering
	\includegraphics[width=2.5in]{Figures/similarity_distr_cp.jpg}
	\caption{U(-$\alpha$, $\alpha$)}
	\label{pics:uniformalpha}
\end{figure}
Figure~\ref{pics:uniformalpha} shows U(-$\alpha$, $\alpha$). The diversity in the society increases as $\alpha$ increases. Each possible similarity value occurs with the same probability as the others. 
