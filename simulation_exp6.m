...
locality = [true, true];
similarity_polarity = 0:0.1:1;
averages = zeros(size(similarity_polarity,2),1);
stds = zeros(size(similarity_polarity,2),1);
t_steady = zeros(size(similarity_polarity,2),1);
i_avg = zeros(size(similarity_polarity,2),1);
for k=1:size(similarity_polarity,2)
    i = 0;
    X_average = 0;
    std_average = 0;
    steadystate = 0;
    connectivity_avg = 0;
    distr = {{'bimodal',similarity_polarity(k)}, {'uniform',[0,1]}, {'uniform',[0,1]}};
...
figure;
plot(similarity_polarity,averages);
title("influenceability level vs Averages")
figure;
plot(similarity_polarity,stds);
